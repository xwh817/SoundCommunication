...
public class FFT {
     ...
     * @return
     */
    public int getFrequency(short[] data, int SAMPLE_RATE, int startIndex, int FFT_N) {

        if (data.length  < FFT_N + startIndex) {
	        Log.e("FFT", "Data length lower than " + FFT_N);
	        return 0;
        }
...
//        Log.i("FFT","max index:"+fmax+" fft:"+f[fmax]+" s:"+s[fmax]);
        int fre = fmax * SAMPLE_RATE / FFT_N;
        Log.i("FFT", "fre:" + fre);
        return fre;
    }
